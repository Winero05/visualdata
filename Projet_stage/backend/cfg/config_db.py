...
# Librairie de connection aux bases de données.
...
class PsqlConfig(BaseModel):
    ...
    # cached_dsn: ClassVar[dict[str, Any]] = {}
    ...
    def load_database_info(cls) -> tuple[list[str], list[str], list[str]]:
        ...
        dbname = os.environ.get("PG_DBNAME", "db_ovdd")
    ...
    def validate_list_fields(
        ...
        #     TypeError(f"Le champ {name} doit être une chaîne de caractère.")

        # if not isinstance(values, Dict):
        #     TypeError(f"Le champ {values} doit être un dictionnaire.")

        # if not isinstance(allowed_values, list):
        #     TypeError(f"Le champ {allowed_values} doit être une liste.")
    ...
    def model_validate_inputs(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Initialisation des variables de cla
        # @classmethodsse.
...
# -----------------------------------------------------------------------------
        ...
        #         values = self.validate_list_fields(
        #             name="dbname",
...
class MysqlConfig(BaseModel):
    ...
    conn_name: Annotated[
        str,
        Field(
            default="connection_mysql",
            description="Le nom qu'il faut donner à cette connection à la BD.",
            examples=["connection_mysql"],
        )
    ] # Définir un nom pour la connection courante.
    ...
    port: Annotated[
        ...
        Field(
            ...
            le=65535,
            examples=["5432"],
            description="Port MySQL entre 1 et 65535 (par défaut 5432).",
        )
    ] # Définir le port de connection.
...
#     def get_config_class(self) -> Type[Union[PsqlConfig, SqlitConfig, MysqlConfig]]:
#         """Cette méthode revoie la classe à configurer en focntion du choix de l'utilisateur.

#         Returns:
#             (PsqlConfig | SqlitConfig | MysqlConfig): Au moins une des classes à configurer est retournée.
#         """
#         if self is DbType.PSQL:
#             return PsqlConfig
#         if self is DbType.SQLIT:
#             return SqlitConfig
#         if self is DbType.MYSQL:
#             return MysqlConfig

# T = TypeVar("T", PsqlConfig, SqlitConfig, MysqlConfig)

# class ConfigDb(Generic[T]):
#     """Gère la configuration d'une base de données en fonction du choix de l'utilisateur
...
#         Args:
#             db_type (DbType): le type de base de données à configurer (PSQL, MYSQL ou SQLITE).
#         """
#         config_class: Type[PsqlConfig] | Type[SqlitConfig] | Type[MysqlConfig] = db_type.get_config_class()
#         self.db_type = db_type
#         self.config: T = cast(T, config_class(**kwargs))
#         # self.config: T = config_class(**kwargs)

#     def get_config(self) -> T:
#         """Renvoie la configuration d'une base de données choisi par l'utilisateur.

#         Returns:
#             T: Une classe configuré est renvoyé.
#         """
#         return self.config
...
class PsqlRequest(BaseModel):
    """Cette classe appelle la classe permettant de faire la configuration de PostgreSQL.

    Args:
        BaseModel (Model): Classe permettant la vérification automatique des entrées utilisateur.
    """
    db_type: Literal["psql"]
    params: PsqlConfig

class MysqlRequest(BaseModel):
    """Cette classe appelle la classe permettant de faire la configuration de MySQL.

    Args:
        BaseModel (Model): Classe permettant la vérification automatique des entrées utilisateur.
    """
    db_type: Literal["mysql"]
    params: MysqlConfig

class SqliteRequest(BaseModel):
    """Cette classe appelle la classe permettant de faire la configuration de SQLite.
...
DbCreateRequest = Annotated[
    Union[PsqlRequest, MysqlRequest, SqliteRequest],
    Field(discriminator='db_type')
]

@dataclass
class DbConfigRegistry:
    """Cette classe enrégistre dans une BD la configuration faite pour chaque base de données.
    """

    _configs: Dict[str, DbCreateRequest] = Field(default_factory=lambda: dict)

    def add(self, config_id: str, config: DbCreateRequest) -> None:
        """Cette méthode ajoute une configuration dans un dictionaire de configuration.

        Args:
            config_id (str): L'identifiant de la configuration.
            config (DbCreateRequest): Le type de configuration (PostgreSQL, MySQL ou SQLite).

        Returns:
            None: Rien n'est retournée une fois la configuraiton faite.
        """
        self._configs[config_id] = config
        return None

    def get(self, config_id: str) -> DbCreateRequest|None:
        """Cette méthode retourne une configuration en fonction de sont identifiant.

        Args:
            config_id (str): L'identifiant de la configuration à chercher.

        Returns:
            DbCreateRequest|None: La configuration est retournée si elle est trouvée via son identifiant sinon rien n'est retournée.
        """
        return self._configs.get(config_id)

    def remove(self, config_id: str) -> None:
        """Cette méthode supprime une configuration via son identifiant.

        Args:
            config_id (str): L'identifiant de la configuration à supprimer.

        Returns:
            DbCreateRequest: La configuration supprimer est retournée.
        """
